export function applyPosterizeToImage(canvas, image, levels = 5.0, edgeMix = 0.12, options = {}) {
  ...
  function draw() {
    ...
    const srcEl = opts.sourceCanvas || image;
    if (opts.updateSource) opts.updateSource();
    gl.viewport(0, 0, canvas.width, canvas.height);
    gl.useProgram(prog);
    gl.uniform2f(uTexel,
      1.0 / (srcEl.naturalWidth || srcEl.width),
      1.0 / (srcEl.naturalHeight || srcEl.height)
    );
    gl.uniform1f(uLevels, levels);
    gl.uniform1f(uEdgeMix, edgeMix);
    gl.uniform1f(uTime, time);
    if (opts.sourceCanvas) { gl.bindTexture(gl.TEXTURE_2D, tex); gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, srcEl); }
