...
async function transitionToScene(sceneIndex) {
  ...
  if (scene.gif && sceneAssets.gif) {
    // --- GIF Scene processed through shader via dynamic offscreen canvas ---
    canvas.style.display = 'block';
    // Ensure the GIF keeps animating by adding a hidden DOM <img> as the live source
    let liveGifEl = null;
    if (gifWrapper) {
      liveGifEl = new Image(); liveGifEl.src = scene.gif; liveGifEl.alt = '';
      liveGifEl.className = 'offscreen-gif'; gifWrapper.appendChild(liveGifEl);
    }
    const dpr = Math.min(window.devicePixelRatio || 1, 1.5);
    const srcCanvas = document.createElement('canvas'); const sctx = srcCanvas.getContext('2d');
    const updateSource = () => {
  ...
  const promises = scenes.map(scene => {
    ...
    if (scene.gif) {
      const gifPromise = new Promise((resolve, reject) => {
        ...
        gif.onerror = (err) => reject(`Failed to load ${scene.gif}: ${err}`);
        gif.src = scene.gif;
      });
      return Promise.all([imagePromise, gifPromise]).then(([img, gif]) => ({ img, gif }));
    }
